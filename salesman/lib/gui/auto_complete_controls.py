...
class NameCtrlWithItemAutoComplete(wx.TextCtrl):
    def __init__(self, parent, backend, showHead=True, dropDownClick=True,
                    hideOnNoMatch=True, selectCallback=None, **therest):
        ...
        self.dropdownlistbox = ItemViewer( self.dropdown, style=flags ,
                                            pos=wx.Point(0,0))
    ...
    def UpdateDisplay(self):
        match = self.GetQueryResult(self._query)
        count = match.count()
        self.match_found = count > 0
    ...
    def _setValueFromSelected( self ) :
         ...
         sel = self.dropdownlistbox.GetFirstSelected()
         if sel > -1:
            item = self.dropdownlistbox.GetItemAt(sel)
            if self._selectCallback:
                self._selectCallback( item )
    ...
    def _showDropDown ( self, show = True ) :
        ...
        if show :
            ...
            if size.GetWidth() != width :
                ...
                s=self.dropdown.GetClientSize()
                self.dropdownlistbox.SetSize(s)
...
class TextCtrlAutoComplete (wx.TextCtrl, listmix.ColumnSorterMixin ):
    ...
    # -- Interfaces methods
    def SetMultipleChoices(self, choices, colSearch=0, colFetch=-1, resizeCol=0):
        ...
        self._setListSize()
        #self.dropdownlistbox.setResizeColumn(resizeCol)
