...
TransactionEditEvent, EVT_TRANSACTION_EDIT = NewCommandEvent()

def PostTransactionEditEvent(target, transaction):
    evt = TransactionEditEvent(target.GetId(), transaction=transaction)
    wx.PostEvent(target, evt)
